...
# #######
tops = df_normalized_both_haps.sort_values("p_values",ascending=True)[0:50]
print(pybedtools.BedTool.from_dataframe(tops.sort_values(["chrom","start"])\
    .loc[:,["chrom","start","stop"]]).merge(d=150000).to_dataframe())
for index, row in tops.iterrows():
    ...
    for sample in samples:
        ...
        repli_sample = repli_sample[(repli_sample["chrom"]==row["chrom"]) 
                                    & (repli_sample["start"]>=row["start"]-2000000)
                                    & (repli_sample["stop"]<=row["stop"]+2000000)]
        print( " repli sample ")
        print(repli_sample)
    ...
    ax.set_xlim(row["start"]-1800000,row["stop"]+1800000)
    ax.set_xticks(np.linspace(row["start"]-1800000,row["stop"]+1800000,5))
