...
bedtools coverage -a $out_dir$filename.tmp3.plus.bed -b $ref_dir/ucsc.L1s.hg38.chr$chrom.bed -F 0.9 |
...
awk 'OFS="\t"{print $1, $2, $3, $4, 0, $6}' $out_dir$filename.intergenic.plus.$first_merge.$second_merge.$filter_size.vlinc.discovery.bed > $out_dir$filename.intergenic.$first_merge.$second_merge.$filter_size.vlinc.discovery.plus.browser.bed
awk 'OFS="\t"{print $1, $2, $3, $4, 0, $6}' $out_dir$filename.intergenic.minus.$first_merge.$second_merge.$filter_size.vlinc.discovery.bed > $out_dir$filename.intergenic.$first_merge.$second_merge.$filter_size.vlinc.discovery.minus.browser.bed
