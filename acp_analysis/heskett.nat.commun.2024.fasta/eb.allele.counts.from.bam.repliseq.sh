...
bcftools mpileup -R VCF3_2_fixed.haplotype.aligned.het.nochr.bed \
...
tail -n +2 $out_dir$filename.table | awk 'OFS="\t"{print $1,$2-1,$2,$3,$4,$5}' |
  bedtools intersect -a stdin -b VCF3_2_fixed.haplotype.aligned.het.nochr.bed -wa -wb > $out_dir$filename.allele.counts.bed
