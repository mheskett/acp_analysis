...
bcftools mpileup -R ACP6_final_bothparents_phased.nochr.bed \
...
tail -n +2 $out_dir$filename.table | awk 'OFS="\t"{print $1,$2-1,$2,$3,$4,$5}' |
  bedtools intersect -a stdin -b ACP6_final_bothparents_phased.hap1hap2.nochr.bed -wa -wb > $out_dir$filename.allele.counts.bed
